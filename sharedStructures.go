...
// See also userinfo.idl: enum UserValueUnits.
...
// Time - When using start and limit to only get a part of all results
...
// OptionalString - A string that can be switched on/off. String is meaningful only if switched on.
// Note: all fields must be assigned if used in set methods
...
// OptionalEntity - IP Address Group / Time Range / ... that can be switched on/off
// Note: all fields must be assigned if used in set methods
...
// LocalizableMessageParameters - Message can contain replacement marks: { "User %1 cannot be deleted.", ["jsmith"], 1 }.
